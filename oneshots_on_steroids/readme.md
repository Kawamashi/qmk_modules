...
One-Shots on Steroids are *nimble*: a OSoS key can be used to activate a layer alongside one or several modifier(s). 

One-Shots on Steroids are *versatile*: three optional behaviours can be activated (and potentially combined) to better suit your needs.
...
One-Shots on Steroids work simply: the modifier or the layer is registered as soon as you press the key, and remains active while the key is held.
...
If the key is released before the One-Shot Term, but a keycode is tapped in between, it behaves like a regular modifier. 
...
To prevent mouse interaction, One-Shot mods on Steroids using Shift and/or Ctrl unregister their modifier(s) as soon as they are released. If the one-shot behaviour is triggered, the modifier will be sent alongside the other key. 
...
Cancelling a one-shot on steroids is easy. First, a timeout can be defined in your `config.h`, to automatically deactivate one-shot keys after a period of keyboard inactivity:
```c
#define OS_STEROIDS_TIMEOUT 3000
```
If you activated a one-shot on steroids by mistake, just tap it again to cancel it. 
...
| `is_oneshot_on_steroids(keycode)`        | Returns whether `keycode` is a one-shot on steroids key.                                  |
| `is_oneshot_layer_on_steroids(keycode)`  | Returns whether `keycode` is a one-shot layer on steroids key.                            |
| `is_oneshot_mod_on_steroids(keycode)`    | Returns whether `keycode` is a modifier-only one-shot on steroids key.                    |
...
| `del_oneshot_mods_on_steroids(mods)`     | Deactivates all one-shot on steroids using `mods`.                                    |
| `clear_oneshot_mods_on_steroids()`       | Deactivates all one-shot on steroids using any modifier.                              |
